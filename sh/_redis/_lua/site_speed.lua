...
if #the_keys == 0 then
  for day = 0, 2 do
    ...
    if redis.call('EXISTS', day_key) then
      table.insert(the_keys, day_key)
    else
      if day > 10 then break end
    end
  end
end
...
for day = 0, 1000 do
  ...
  for _, requested_key in pairs(the_keys) do
    redis.call('SINTERSTORE', 'day:current:slug_', day_key, requested_key)

    log('Processing '..day_key..', '..requested_key..': '..redis.call('SCARD', 'day:current:slug_'))
    if redis.call('SCARD', 'day:current:slug_') > 0 then

      --redis.call('DEL', 'ppers:slug_', 'onramp:slug_')
